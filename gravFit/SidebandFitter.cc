...
RooWorkspace* SidebandFitter::getAlphaFit( int btagCategory, const std::string& leptType_str, TTree* treeMC ) {
  ...
  treeMC->SetBranchAddress("mZZ",&mZZ);
  treeMC->SetBranchAddress(PUType_.c_str(),&eventWeight);
  treeMC->SetBranchAddress("nBTags",&nBTags);
  treeMC->SetBranchAddress("mJJ",&mZjj);
  treeMC->SetBranchAddress("lep",&leptType);
  
  float bins0[17]={280,300,320,340,360,380,400,430,460,490,520,550,600,650,700,800,1000};

  TH1D h1_mZZ_signalRegion("mZZ_signalRegion", "", 16, bins0);
  h1_mZZ_signalRegion.Sumw2();
  TH1D h1_mZZ_sidebands("mZZ_sidebands", "", 16, bins0);
  ...
  h1_alpha.Draw("F");
  char canvasName[400];
  sprintf( canvasName, "%s/mZZ_alpha_%dbtag_%s.eps", outdir.c_str(), btagCategory, leptType_str.c_str());
  c1->SaveAs(canvasName);
  RooWorkspace* alphafit=new RooWorkspace();
  return alphafit;
}
