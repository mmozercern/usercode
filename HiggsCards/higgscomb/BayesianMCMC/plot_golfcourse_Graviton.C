...
void plot_golfcourse_Graviton(){
  ...
  TFile *fFREQ=new TFile("higgsCombineGrav2L2Q_MCMC.RSGrav05.20120209.root","READ");
 ...
 //read in theoretical values from text files
  ...
  string xsect_file_interpol="./RSGravXSectTimesBRToZZ_Pythia_c05_LININTERP.txt";
  ...
  TGraphAsymmErrors *grthSM=new TGraphAsymmErrors(nMassEff1,mass1,xs,0,0,xs_downerr,xs_uperr);//xs_downerr,xs_uperr);
  grthSM->SetName("SMXSection");

 
 
  // cout<<"Plotting"<<endl;
  double fr_left=290.0, fr_down=0.01,fr_right=1010.0,fr_up=5.0;
 ...
 // gStyle->SetOptStat(0);
  ...
  //more graphics
  TLegend *leg = new TLegend(.48,.76,.94,.94);
  //   TLegend *leg = new TLegend(.35,.71,.90,.90);
   ...
   //   leg->SetBorderMode(0);
   leg->AddEntry(grobslim_cls, "MCMC Observed", "LP");
   leg->AddEntry(gr68_cls, "MCMC Expected #pm 1#sigma", "LF");
   leg->AddEntry(gr95_cls, "MCMC Expected #pm 2#sigma", "LF");
