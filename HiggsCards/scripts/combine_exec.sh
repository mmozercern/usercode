...
myrand=$1
mass=$2
OUTDIR=datacards_20110811
echo "Starting HiggsCombination with seed=$myrand at $( date +%c ) on $hostname."

startdir=$( pwd )

#set CMSSW environment
RELDIR=/afs/cern.ch/user/s/sbologne/scratch0/CMSSW/CMSSW_4_2_4/src/
...
label="2l2q"
ntoys=5
WORKDIR=${RELDIR}/HiggsAnalysis/CombinedLimit/test/rotatedEpsHqtSyst/${OUTDIR}/${mass}/
...
#expected
combine -M $algo -n $label -m $mass  -s $myrand -d $WORKDIR/$datacard  -H $hint  -t $ntoys -U -L  /afs/cern.ch/user/s/sbologne/scratch0/CMSSW/CMSSW_4_2_4/src/HiggsAnalysis/CombinedLimit/test/rotatedEps/PDFs/RooCB_cc.so 
#observed
