...
process.source = cms.Source("PoolSource", 
    ...
    'file:F0D822C6-4D47-DF11-9590-0030487E5179.root'
...
process.p = cms.Path( process.genJetParticles  + process.ak5GenJets + process.ak7GenJets +process.patDefaultSequence + process.VBSequencePre  +process.VBSequence )


from PhysicsTools.PatAlgos.tools.helpers import *

for i in range(0,50):
    cloneProcessingSnippet(process,process.VBSequence,str(i))
    getattr(process,"JetCleanerForCounting"+str(i)).checkOverlaps.electrons.deltaR=cms.double(i*0.03)
    setattr(process,"p"+str(i),cms.Path(process.genJetParticles  + process.ak5GenJets + process.ak7GenJets +process.patDefaultSequence + process.VBSequencePre  + getattr(process,"VBSequence"+str(i))) )
